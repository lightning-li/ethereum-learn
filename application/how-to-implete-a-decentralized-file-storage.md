...
译者注：已知信道会错误传输 `k` 这个值，那么 `P(x)` 可以通过传输其上的 `n + 2k` 个点来纠正其中 `k`个错误，因此上述函数第一个参数中 6 个点中可以允许 1 个错误的点，并可以纠正它。
...
现在，现实中，这些相对简单的取余有限域并不是我们真正需要用在错误检测代码中的。更喜爱的通用构建方法被称作[伽罗瓦域](https://en.wikipedia.org/wiki/Galois_field) (技术上，任何拥有有限个数元素的域都可以称之为伽罗瓦域，但是有时候这个术语被用来更具体地指向基于多项式的域，就正如我们将要描述的)。思想是，域中的元素现在是多项式，系数是整数域中的自身值对 2 取余 (例，`a + b := (a + b) % 2`，等等)。加法和减法就像正常的那样工作，但是乘法是它自身对某一个多项式取余，具体点来说，是 `x^8 + x^4 + x^3 + x + 1`。这个更为复杂的多层构建让我们拥有了一个恰好有 256 个元素的域。如果我们想一次性地在多个字节上应用，我们仅需要并行地应用该模式 (例，如果每一块是 1024 个字节，决定 10 个多项式，每一个对应一个字节，分开扩展它们，然后结合在多项式在每一个 x 轴处的值来得到附加块)。

**译者注：**

---
本人认为如果每一个快对应 1024 个字节，应该使用 1024 个多项式，每一个多项式对应一个字节；举一个简单的例子来说，假设一个块对应 4 个字节，现在需要 4 个多项式，假设文件内容是 "abcdefghijkl"，那分割该文件为 2 块，块 1 为 "abcd"，块 2 为 "efgh"，块 3 为 "ijkl"。那第一个多项式由 `(1, a)`、`(2, e)`、`(3, i)` 决定；第二个多项式由 `(1, b)`、`(2, f)`、`(3, j)` 决定，以此类推。
...
无论从哪一种方向修复，我们都得到了 `6`。这是一件令人惊奇的事情：多项式在 x 和 y 轴上工作地一样好。因此，如果我们从该网格中使用这 16 块，将它们分散到 16 个不同的节点上，
